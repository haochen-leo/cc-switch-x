...
| `apply_patch` 协议       | Codex custom tool <-> native Responses upstream | 严格第三方网关不接受 `type=custom`，或把 patch 包进 JSON 字段                     | 请求 400/422；响应 `function_call` 无法被 Codex 当作 file change 执行 | 请求 custom declaration/choice/history 转标准 function；响应 function 非流式/SSE 还原 custom；最后解包 raw patch | `src-tauri/src/proxy/providers/transform_codex_apply_patch.rs`、`src-tauri/src/proxy/forwarder.rs`、`src-tauri/src/proxy/handlers.rs` | 第三方 Native 自动；OpenAI/DeepSeek 官方直通 | `cargo test --manifest-path src-tauri/Cargo.toml transform_codex_apply_patch -- --nocapture`；真实 Codex CLI + Qwen file_change |

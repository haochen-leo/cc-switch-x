...
The storage location for CC Switch's own data, defaulting to `~/.cc-switch-x/`.
...
Configure CC Switch X's outbound HTTP/HTTPS proxy for access to external APIs. When an address is set, that proxy is used explicitly. When left empty, CC Switch X follows the system or environment proxy. A system proxy pointing to CC Switch X's own local-routing port is bypassed automatically to prevent a proxy loop.

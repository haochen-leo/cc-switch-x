...
Current CC Switch X beta macOS builds are not signed or notarized with an X-specific Apple certificate yet, so macOS may show a Gatekeeper warning. Download only from the [Releases page](https://github.com/haochen-leo/cc-switch-x/releases), verify the source, and then choose to open it.
...
1. Check if the `~/.cc-switch-x/` directory exists
2. Restore from backup: `~/.cc-switch-x/backups/`
...
2. Restart the app
3. Delete `~/.cc-switch-x/settings.json` to reset settings
...
Yes. Upstream CC Switch v3.13.0 introduced coverage for all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return), and CC Switch X retains that behavior. Clicking a `ccswitchx://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/haochen-leo/cc-switch-x/issues)
...
CC Switch stores logs under its application configuration directory. By default this is `.cc-switch-x` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.cc-switch-x/logs/cc-switch-x.log` and its rotated files
- Application crashes: `~/.cc-switch-x/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.cc-switch-x\...`

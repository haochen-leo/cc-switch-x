# CC Switch X Flatpak Build Guide

> **Status**: inherited from upstream CC Switch and **not yet validated or
> published for CC Switch X**. The manifest still uses the upstream Flatpak
> identifier and package names. Do not use it for an X release until the ID,
> binary name, metadata, filesystem permissions, and build artifacts have been
> renamed and tested together.

This directory contains the inherited Flatpak manifest (`com.ccswitch.desktop`)
for CC Switch, used to convert the generated `.deb` artifact into an
installable `.flatpak` package via CI or local builds.

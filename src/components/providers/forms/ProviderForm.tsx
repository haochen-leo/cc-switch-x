...
function getClaudeRoutingProviderSlotValues(
  ...
  return {
    ...
    sonnetModel: getProviderEnvString(
      provider,
...
function hasClaudeModelRoutingValues(
...
function ProviderFormFull({
  ...
  const { status: proxyStatusData } = useProxyStatus();
  ...
  const handleSubmit = async (values: ProviderFormData) => {
    ...
    if (appId === "claude") {
      ...
      if (hasClaudeRouting) {
        ...
        if (
          rectifierConfig &&
          (!rectifierConfig.enabled || !rectifierConfig.requestToolUseId)
        ) {
          issues.push(

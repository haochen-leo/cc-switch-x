...
pub const CC_SWITCH_CODEX_OFFICIAL_PROXY_PROVIDER_ID: &str = "cc-switch-x-official";
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "cc-switch-x-model-catalog.json";
...
fn is_local_codex_proxy_url(url: &str) -> bool {
    let url = url.trim();
...
pub fn codex_config_has_official_proxy_route(config_text: &str) -> bool {
    ...
    let active_provider_id = doc.get("model_provider").and_then(|item| item.as_str());
    if active_provider_id == Some(CC_SWITCH_CODEX_OFFICIAL_PROXY_PROVIDER_ID) {
        return true;
    }
    if active_provider_id != Some(CC_SWITCH_CODEX_MODEL_PROVIDER_ID) {
        return false;
    }
    doc.get("model_providers")
...
/// `custom` routes keep their history identity; last-resort cleanup only strips
/// their local proxy base URL before this function is called.
...
///
/// 两种情况拒绝注入、原样返回：
/// - 配置已有非内建 `openai` 的显式 `model_provider`：用户手工指定的路由不被覆盖；
...
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if let Some(configured_provider) = doc.get("model_provider") {
        if configured_provider.as_str() != Some(CODEX_OPENAI_MODEL_PROVIDER_ID) {
            return Ok(config_text.to_string());
        }
    }
...
mod tests {
    ...
    fn seed_rotated_managed_codex_live_state() -> CodexLiveTestState {
        ...
        crate::config::write_text_file(
            ...
            "# cas-guard-sentinel\nmodel = \"gpt-5.5\"\nmodel_catalog_json = \"cc-switch-x-model-catalog.json\"\n",
        )
...
"#;
    ...
    #[test]
    fn official_proxy_route_uses_legacy_provider_when_unified_history_is_disabled() {
...
"#;
    ...
    #[test]
    fn official_proxy_route_uses_custom_when_unified_history_is_enabled() {
...
"#;
        let output = apply_codex_official_proxy_route(input, "http://127.0.0.1:15721/v1", true)
        ...
        assert!(providers
            ...
            .is_none());
        let provider = &providers[CC_SWITCH_CODEX_MODEL_PROVIDER_ID];
        assert_eq!(
            provider.get("base_url").and_then(toml::Value::as_str),
            Some("http://127.0.0.1:15721/v1")
        );
...
"#;
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        ...
        let built_in = "model_provider = \"openai\"\nmodel = \"gpt-5.4\"\n";
        let normalized = inject_codex_unified_session_bucket(built_in).expect("normalize openai");
...
"#;
        ...
        let built_in_with_stale = format!("model_provider = \"openai\"\n\n{stale}");
        let unchanged = inject_codex_unified_session_bucket(&built_in_with_stale).expect("inject");
        assert_eq!(unchanged, built_in_with_stale);
...
"#;
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let base = PathBuf::from("/tmp/.codex");
        let config = r#"model_catalog_json = "/tmp/.codex/cc-switch-x-model-catalog.json"
"#;
...
"#;
        ...
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\cc-switch-x-model-catalog.json");
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/cc-switch-x-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/cc-switch-x-model-catalog.json"
"#;
...
"#;
    ...
    fn resolve_catalog_rejects_absolute_path_outside_config_dir() {
        let config_text = r#"model_catalog_json = "/tmp/secret/cc-switch-x-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_accepts_absolute_path_inside_config_dir() {
        let config_text = r#"model_catalog_json = "/home/user/.codex/cc-switch-x-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_traversal_to_parent_directory() {
        let config_text = r#"model_catalog_json = "../cc-switch-x-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_symlink_escaping_config_dir() {
        ...
        // "link/cc-switch-x-model-catalog.json" 词法上在 base 内，真实读取却落到
        ...
        let config_text = r#"model_catalog_json = "link/cc-switch-x-model-catalog.json"
"#;
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
            result, None,

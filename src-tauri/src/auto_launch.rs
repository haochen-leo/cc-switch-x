...
/// 将 `/path/to/CC Switch For IdealLAB.app/Contents/MacOS/CC Switch For IdealLAB` 转换为 `/path/to/CC Switch For IdealLAB.app`
#[cfg(target_os = "macos")]
...
fn get_auto_launch() -> Result<AutoLaunch, AppError> {
    let app_name = "CC Switch For IdealLAB";
    let exe_path =
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new(
            "/Applications/CC Switch For IdealLAB.app/Contents/MacOS/CC Switch For IdealLAB",
        );
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Applications/CC Switch For IdealLAB.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new(
                "/Users/test/My Apps/CC Switch For IdealLAB.app/Contents/MacOS/CC Switch For IdealLAB",
            );
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/CC Switch For IdealLAB.app"
            ))
        );
    }

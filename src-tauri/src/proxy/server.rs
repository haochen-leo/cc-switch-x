...
impl ProxyServer {
    ...
    fn build_router(&self) -> Router {
        Router::new()
            ...
            .route("/files", post(handlers::handle_codex_files))
            .route("/v1/files", post(handlers::handle_codex_files))
            .route("/v1/v1/files", post(handlers::handle_codex_files))
            .route("/codex/v1/files", post(handlers::handle_codex_files))
            .route(
                "/files/:file_id/uploaded",
                post(handlers::handle_codex_files),
            )
            .route(
                "/v1/files/:file_id/uploaded",
                post(handlers::handle_codex_files),
            )
            .route(
                "/v1/v1/files/:file_id/uploaded",
                post(handlers::handle_codex_files),
            )
            .route(
                "/codex/v1/files/:file_id/uploaded",
                post(handlers::handle_codex_files),
            )
            .route(
                "/realtime/calls",
                post(handlers::handle_codex_realtime_calls),
            )
            .route(
                "/v1/realtime/calls",
                post(handlers::handle_codex_realtime_calls),
            ...
            .route(
                "/realtime",
                any(handlers::handle_codex_realtime_websocket_unsupported),
            )
            .route(
                "/v1/realtime",
                any(handlers::handle_codex_realtime_websocket_unsupported),

...
mod tests {
    ...
    #[test]
    fn test_request_ultra_effort_clamps_to_max_effort() {
        // ultra is a Codex extension level; Anthropic's effort API tops out at
        // max. Compatible gateways reject legacy budget_tokens, so preserve the
        // highest intent through output_config without emitting a token budget.
        let input = json!({
            "model": "c",

...
//! here.
//!
//! This combines sub2api's `patchGrokResponsesBody` with xAI-specific schema,
//! collaboration `agent_message`, unknown-model, and whole-float argument
//! compatibility. Gated by `provider_needs_xai_responses_sanitize`, covering xAI
//! OAuth and API-key cards whose live upstream is `api.x.ai` Responses. Namespace
//! flattening remains a separate generic third-party transform.
//!
//! Run request sanitizers after namespace flattening so lifted `function` tools
//! survive the strict xAI tool-type whitelist. Keep every xAI-only rewrite in
//! this file so upstream equivalents can replace this layer cleanly.

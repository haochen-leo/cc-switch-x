            ...
            conn.query_row(
                ...
                |row| {
                    Ok(AppProxyConfig {
                        ...
                        retry_429_enabled: row.get::<_, i32>(4)? != 0,
                        retry_429_max_retries: row.get::<_, i64>(5)? as u32,
                        retry_429_initial_delay_ms: row.get::<_, i64>(6)? as u64,
                        streaming_first_byte_timeout: row.get::<_, i32>(7)? as u32,
                        streaming_idle_timeout: row.get::<_, i32>(8)? as u32,
                        non_streaming_timeout: row.get::<_, i32>(9)? as u32,
                        circuit_failure_threshold: row.get::<_, i32>(10)? as u32,
                        circuit_success_threshold: row.get::<_, i32>(11)? as u32,
                        circuit_timeout_seconds: row.get::<_, i32>(12)? as u32,
                        circuit_error_rate_threshold: row.get(13)?,
                        circuit_min_requests: row.get::<_, i32>(14)? as u32,
                    })
                },
            )
        };

...
///   仍用旧价，下一个同步 pass 生效。
/// - `route_models`: aggregate route aliases resolved once for consistent model attribution.
...
fn sync_single_codex_file(
    ...
    let parsed = parse_codex_file(
        file_path,
        thread_id_from_filename(file_path),
        &pass.route_models,
    )?;
...
mod tests {
    ...
    fn test_interleaved_counter_lanes_use_exact_last_usage() -> Result<(), AppError> {
        ...
        let parsed = parse_codex_file(&file, Some(PARENT_ID.to_string()), &HashMap::new())?;
        let deltas = parsed
            .token_events
            .iter()

...
impl ProxyService {
    ...
    fn write_codex_takeover_live_for_provider(
        ...
        let managed_official = native_auth_passthrough && managed_account_id.is_some();
...
"#
        ...
        assert_eq!(
            model_provider(&official_live).as_deref(),
        ...
        assert_eq!(
            model_provider(&third_party_live).as_deref(),
            Some(crate::codex_config::CC_SWITCH_CODEX_MODEL_PROVIDER_ID)
        );
        ...
        assert_eq!(
            model_provider(&restored_live).as_deref(),
            Some(crate::codex_config::CC_SWITCH_CODEX_MODEL_PROVIDER_ID)
        );
...
"#;
    ...
    #[test]
    fn apply_codex_proxy_toml_config_unifies_builtin_official_history() {
        let mut provider = Provider::with_id(
            "codex-official".to_string(),
            "OpenAI Official".to_string(),
            json!({ "auth": {}, "config": "" }),
        ...
        let parsed: toml::Value = toml::from_str(&output).expect("valid unified official route");
...
"#;
    ...
    async fn hot_switch_provider_updates_claude_live_while_preserving_takeover_fields() {
        ...
        let expected_proxy_url = format!("http://127.0.0.1:{}", crate::brand::DEFAULT_PROXY_PORT);
        assert_eq!(
            live.get("env")
                .and_then(|env| env.get("ANTHROPIC_BASE_URL"))
                .and_then(|v| v.as_str()),
...
"#
        ...
        assert_eq!(
            parsed_live
                ...
                .and_then(|v| v.get("base_url"))
                .and_then(|v| v.as_str()),
            Some(format!("http://127.0.0.1:{}/v1", crate::brand::DEFAULT_PROXY_PORT).as_str()),
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "cc-switch-x-model-catalog.json must be created on provider switch"
        );
        ...
        assert!(
            message.contains("写入 Codex 配置失败")
                ...
                || (message.contains("捕获 Codex 热切换前状态失败")
                    && message.contains("cc-switch-x-model-catalog.json")),
